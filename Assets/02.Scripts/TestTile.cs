...
public class TestTile : MonoBehaviour
{
    ...
    public Vector3 NodeToPosition(TestIntVector2 nodePos, TestIntVector2 size, EFitType fitType)
    {
        ...
        TestIntVector2 lastPos;
        switch (fitType)
        {
            case EFitType.MXPYFits: // 왼쪽 위로 배치 가능
                lastPos = new TestIntVector2(nodePos.x - sizeValue.x, nodePos.y + sizeValue.y);
                break;
            ...
            case EFitType.EXPYFits: // 오른쪽 끝으로 위로 배치 가능
                lastPos = new TestIntVector2(_dimensions.x - 1, nodePos.y + sizeValue.y);
            ...
            case EFitType.PXEYFits: // 오른쪽 위 끝으로 배치 가능
                lastPos = new TestIntVector2(nodePos.x + sizeValue.x, _dimensions.y - 1);
            ...
            case EFitType.EXEYFits: // 오른쪽 끝 위 끝으로 배치 가능
                lastPos = new TestIntVector2(_dimensions.x - 1, _dimensions.y - 1);
    ...
    public EFitType Fits(TestIntVector2 gridPos, TestIntVector2 size)
    {
        ...
        {
            bool overlaps = false;
            TestIntVector2 extents = new TestIntVector2(gridPos.x + size.x, gridPos.y - (size.y - 1));
            for (int y = gridPos.y; y >= extents.y; y--)
            {
                for (int x = gridPos.x; x < extents.x; x++)
                {
        ...
        {
            bool overlaps = false;
            TestIntVector2 extents = new TestIntVector2(gridPos.x - (size.x - 1), gridPos.y + size.y);
            for (int y = gridPos.y; y < extents.y; y++)
            {
                for (int x = gridPos.x; x >= extents.x; x--)
                {
        ...
        {
            bool overlaps = false;
            TestIntVector2 extents = gridPos - new TestIntVector2(size.x - 1, size.y - 1);
            for (int y = gridPos.y; y >= extents.y; y--)
            {
                for (int x = gridPos.x; x >= extents.x; x--)
                {
        ...
        {
            bool overlaps = false;
            TestIntVector2 extents = new TestIntVector2(_dimensions.x - size.x, gridPos.y + size.y);
            for (int y = gridPos.y; y < extents.y; y++)
            {
        ...
        {
            bool overlaps = false;
            TestIntVector2 extents = new TestIntVector2(_dimensions.x - size.x, gridPos.y - (size.y - 1));
            for (int y = gridPos.y; y >= extents.y; y--)
            {
                for (int x = _dimensions.x - 1; x >= extents.x; x--)
                {
        ...
        {
            bool overlaps = false;
            TestIntVector2 extents = new TestIntVector2(gridPos.x + size.x, _dimensions.y - size.y);
        ...
        {
            bool overlaps = false;
            TestIntVector2 extents = new TestIntVector2(gridPos.x - (size.y - 1), _dimensions.y - size.y);
            for (int y = _dimensions.y - 1; y >= extents.y; y--)
            {
                for (int x = gridPos.x; x >= extents.x; x--)
                {
        ...
        {
            bool overlaps = false;
            TestIntVector2 extents = new TestIntVector2(_dimensions.x - size.x, _dimensions.y - size.y);
            for (int y = _dimensions.y - 1; y >= extents.y; y--)
            {
                for (int x = _dimensions.x - 1; x >= extents.x; x--)
                {
    ...
    public void Occupy(TestIntVector2 gridPos, TestIntVector2 size, EFitType fitType)
    {
        TestIntVector2 extents;
        switch (fitType)
        {
            ...
            case EFitType.EXEYFits:
                extents = new TestIntVector2(_dimensions.x - 1, _dimensions.y - 1);
                for (int y = extents.y; y >= _dimensions.y - size.y; y--)
                {
                    for (int x = extents.x; x >= _dimensions.x - size.x; x--)
                    {
    ...
    public void Clear(TestIntVector2 gridPos, TestIntVector2 size, EFitType fitType)
    {
        TestIntVector2 extents;
        switch (fitType)
        {
