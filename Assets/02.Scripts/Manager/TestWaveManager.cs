...
public class TestWaveManager : MonoBehaviour
{
    ...
    private void Start()
    {
        TestPoolManager.Instance.WaveEnemyInit(_waves);
    ...
    IEnumerator SpawnWave()
    {
        ...
        for (int i = 0; i < nowWave._spawnDatas.Length; i++)
        {
            ...
            GameObject enemy = TestPoolManager.Instance.PoolGetObject(nowWave._spawnDatas[i]._enemyData.fileName);
            enemy.transform.position = _startPoint.position;
            enemy.GetComponent<TestEnemy>().Active();
            TestGameUI.Instance.AppearanceEnemy(nowWave._spawnDatas[i]._enemyData.enemyName);
        }
        yield return null;
